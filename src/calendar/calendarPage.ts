...
import { estimateFertility } from "../cycle/fertility";
import { vietnamHolidays } from "../events/vietnamHolidays";
...
export function renderCalendarPage(context: CalendarPageContext): string {
  ...
  const holidays = new Map(vietnamHolidays(year).map((holiday) => [holiday.date, holiday.title]));
  ...
  const days = calendarDays.map((day) => {
    ...
    const fertility = estimateFertility(context.periods, prediction, day.date);
    const labels = [formatCalendarDate(day.date), fertility.label, holidays.get(day.date) ?? "", day.date === currentToday ? "hôm nay" : "", selected ? "đã chọn" : ""];
    return `<div class="calendar-cell" role="gridcell" aria-selected="${selected}"><button class="calendar-day ${day.inMonth ? "" : "outside-month"} fertility-${fertility.status} ${actual ? "actual-period" : ""} ${predicted ? "predicted-period" : ""} ${expected ? "expected-period" : ""} ${day.date === currentToday ? "today" : ""} ${selected ? "selected" : ""}" type="button" data-calendar-date="${day.date}" aria-label="${labels.filter(Boolean).join(", ")}"${day.date === currentToday ? " aria-current=\"date\"" : ""} aria-describedby="calendar-legend"><span>${Number(day.date.slice(8, 10))}</span>${holidays.has(day.date) ? "<i class=\"calendar-holiday-dot\"></i>" : ""}</button></div>`;
  });
  ...
  const selectedEstimate = state.selected === null ? undefined : estimateFertility(context.periods, prediction, state.selected);
  ...
  const sheet = editing || state.selected !== null
    ...
    : "";
  return `<section class="calendar-page"><header class="calendar-header"><div><p class="eyebrow">Chu kỳ & tham khảo</p><h1>Lịch của bạn</h1></div><button id="calendar-today" class="secondary-button" type="button">Hôm nay</button></header><section class="calendar-surface" aria-label="Lịch tháng"><div class="calendar-month-controls"><button type="button" data-month-shift="-1" aria-label="Tháng trước">‹</button><h2 id="calendar-month-label">Tháng ${month}, ${year}</h2><button type="button" data-month-shift="1" aria-label="Tháng sau">›</button></div><div class="calendar-weekdays" role="row">${weekdays.map((day) => `<span role="columnheader">${day}</span>`).join("")}</div><div class="calendar-grid" role="grid" aria-labelledby="calendar-month-label">${calendarRows}</div><p id="calendar-legend" class="calendar-legend"><span class="legend-actual">Kỳ kinh đã ghi nhận</span><span class="legend-predicted">Khoảng dự kiến</span><span class="legend-expected">Ngày rụng trứng dự kiến</span><span class="legend-today">Hôm nay</span></p></section><section class="today-summary" aria-labelledby="today-summary-title"><h2 id="today-summary-title">${state.selected ? `Ngày ${formatCalendarDate(state.selected)}` : "Hôm nay"}</h2><p>${escapeHtml(state.selected && selectedEstimate ? selectedEstimate.label : summary)}</p>${state.selected && holidays.get(state.selected) ? `<small>${escapeHtml(holidays.get(state.selected)!)}</small>` : ""}</section><section class="period-history" aria-labelledby="period-history-title"><div><h2 id="period-history-title">Lịch sử kỳ kinh</h2><button id="period-add" class="text-button" type="button">Thêm</button></div>${context.loading ? "<p>Đang tải lịch sử…</p>" : context.error ? "<p>Không thể tải lịch sử. <button id=\"period-retry\" class=\"text-button\" type=\"button\">Thử lại</button></p>" : history ? `<ul>${history}</ul>` : "<p>Chưa có kỳ kinh nào được ghi.</p>"}</section>${sheet}${state.undo ? `<div class="undo-bar" role="status">Đã xóa kỳ kinh.<button id="period-undo" type="button">Hoàn tác</button></div>` : ""}</section>`;
}
