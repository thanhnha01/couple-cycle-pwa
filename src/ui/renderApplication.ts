...
function home(context: RenderContext, membership: CoupleMembership): string {
  ...
  const completedThisWeek = context.sharedTasks.filter((task) => task.completed && task.completedAt && task.completedAt >= Date.now() - 7 * 24 * 60 * 60 * 1000).length;
